...
class PreferencesController extends AppController {
    ...
    public function add() {
        ...
        $pref_count = count($preferences);
        $flag = $this->isPreferenceFillingOpen();
        $specialTime = new \Cake\I18n\FrozenTime('2018-07-17');
        if ($this->request->is(['patch', 'post', 'put']) && $flag === true && ($pref_count == 0 || $preferences[0]->created >= $specialTime)) {
		//if ($this->request->is(['patch', 'post', 'put']) && $flag === true) {
            ...
            for ($count=0; $count < 3; $count++ ) {
                ...
                if(($count == 0) ? 1 : (!empty($pref_data['selected'])) ? 1 : 0) {
                    ...
                    $preference = $this->Preferences->patchEntity($preference, ['user_id' => $this->Auth->user('id'), 'selected' => 1, 'second_round' => 2], ['validate' => false]);
                }
                else {
                    $newData = [];
           ...
           //debug($validationErrosPresent ); debug($allPrefSaved); return null;
            ...
            $this->Flash->error(__('The filling of preferences is open. To change preferences click on reset link and then fill the preferences again.'));
        }
	...
	public function clearPref() {
		...
		foreach($preferences as $pref) {
			$result = $this->Preferences->delete($pref);
		}
		$this->Flash->success(__('Your form has been reset.'));
        return $this->redirect(['action' => 'add']);
	}
	...
	//return parent::isAuthorized($user);
        ...
        if (isset($user['role']) && $user['role'] === 'student' && ($this->request->getParam('action') === 'add' 
                || $this->request->getParam('action') === 'index' || $this->request->getParam('action') === 'viewresult' || $this->request->getParam('action') === 'clearPref')) {
            return true;
        }
