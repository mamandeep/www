...
class RegistrationsController extends AppController {
	...
	$user = $this->Registrations->newEntity();
        ...
        if ($this->request->is(['post', 'put']) && $flag === true) {
            ...
            $match = preg_match('/^PG18[0-9]{7}$/', $user['username']);
	    //debug($match); 
	    $username = "";
	    if($match === 1) {
	    ...
	    if(count($existing) === 1) {
		$this->Flash->success(__('You have already registered successfully. Please login using your Applicant ID (PG18xxxxxxx) and Password.'));
    ...
    public function forgotpasswd()
    {
        if($this->request->is(['post'])) {
           ...
           $session->write('otp',$this->ozekiOTP());
            ...
            if(empty($response) || !$usersTable->save($user)) {
