...
class SeatsController extends AppController {
    ...
    public function locksupranumeraryseat($AuthId = null) {
        $flag = $this->isLockingSeatOpen();
        if(!$flag) {
            $this->Flash->error(__('The Locking of Supranumerrary Seat is closed at this time.'));
            $this->set('lockseatOpen', false);
        }
        else {
            $this->set('lockseatOpen', true);
        ...
        $candidate = $candidatesTable->find('all', ['fields' => ['id','pwd', 'ward_of_def'], 'conditions' => ['Candidates.user_id' => $this->Auth->user('id')]])->toArray();
        $candidate_id = '';
		$pwd = '';
		$wardOfDef = '';
        if(count($candidate) === 1) {
            $candidate_id = $candidate[0]['id'];
	    $pwd = $candidate[0]['pwd'];
	    $wardOfDef = $candidate[0]['ward_of_def'];
        }
        else {
            $this->Flash->error(__('There is an error in you application. Please contact support.'));
            $this->set('lockseatOpen', false);
        }
        $lockseatsTable = TableRegistry::get('Locksupranumeraryseats');
        $rankingsTable = TableRegistry::get('Rankings');
        $seatOptions = $rankingsTable->find('all', ['conditions' => ['Rankings.candidate_id' => $candidate_id, 'Rankings.offered_seat is' => NULL],
                'contain' => ['Programmes', 'Categories']]);
        $rankingSelected = '';
        $lockedSeat = $lockseatsTable->find('all', ['conditions' => ['Locksupranumeraryseats.candidate_id' => $candidate_id]])->toArray();
        if(count($lockedSeat) > 1) {
            $this->Flash->error('An error has occured while locking seat. Please contact support.');
            return null;
        }
        ...
        if ($this->request->is(['patch', 'post', 'put']) && !empty($this->request->data()['selected_course']) && $flag === true) {
            ...
            $lockSeatData = [];
            $pwd_new = $this->request->data()['pwd'];
            $ward_of_def_new = $this->request->data()['wardofdef_category'];
            $rankId = $this->request->data()['selected_course'];
            $rankingObjectArr = $seatOptions->toArray();
            $rankingSelected = $rankingsTable->find('all', ['conditions' => ['Rankings.id' => $rankId]])->toArray();
            $rankingSelected = $rankingSelected[0];
            ...
            $lockSeatData['rank_id'] = $rankingSelected->id;
            $lockSeatData['user_id'] = $rankingSelected->user_id;
            $lockSeatData['pwd'] = $pwd_new;
            $lockSeatData['wardofdef_category'] = $ward_of_def_new;
            $lockedSeat = $lockseatsTable->patchEntity($lockedSeat, $lockSeatData);
            //debug($lockedSeat); 
            if (!($pwd_new == "0" && $ward_of_def_new == "0") && $lockseatsTable->save($lockedSeat)) {
                $this->Flash->success('The seat has been locked.');
            }
            else {
                $this->Flash->error('The seat could not be locked. Please select appropriate field values or contact support.');
            }
        }
        else if($this->request->is(['patch', 'post', 'put']) && empty($this->request->data()['selected_course'])) {
            $this->Flash->error('Please select the radio button under "Lock Seat" and then submit.');
        ...
        $this->set('lockedSeat', $lockedSeat);
	$this->set('pwd', $pwd);
	$this->set('ward_of_def', $wardOfDef);
    }

    
    public function lockspecialcategoryseat($AuthId = null) {
        $flag = $this->isLockingSeatOpen();
        if(!$flag) {
            $this->Flash->error(__('The Locking of Special Drive Seat is closed at this time.'));
            $this->set('lockseatOpen', false);
        }
        else {
            $this->set('lockseatOpen', true);
        ...
        //$this->request->allowMethod(['post', 'delete']);
        $candidatesTable = TableRegistry::get('Candidates');
        $candidate = $candidatesTable->find('all', ['fields' => ['id','pwd', 'ward_of_def', 'category_id'], 'conditions' => ['Candidates.user_id' => $this->Auth->user('id')]])->toArray();
        $candidate_id = '';
	$candidate_category = '';
	$pwd = '';
	$wardOfDef = '';
	    ...
	    $pwd = $candidate[0]['pwd'];
	    $wardOfDef = $candidate[0]['ward_of_def'];
	    $candidate_category = $candidate[0]['category_id'];
        }
        else {
            $this->Flash->error(__('There is an error in you application. Please contact support.'));
            $this->set('lockseatOpen', false);
        ...
        $lockedSeat = (count($lockedSeat) === 0) ? $lockseatsTable->newEntity() : $lockedSeat[0];
        if ($this->request->is(['patch', 'post', 'put']) && !empty($this->request->data()['selected_course']) && $flag === true && $candidate_category != 2 && false) {
            //debug($this->request->data()); return null;
            $lockSeatData = [];
            $rankId = $this->request->data()['selected_course'];
            $rankingObjectArr = $seatOptions->toArray();
	...
	$this->set('pwd', $pwd);
	$this->set('ward_of_def', $wardOfDef);
	$this->set('c_category', $candidate_category);
    }
	...
	public function candidatedetails() {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
			//debug($this->request->data()); return null;
			$cucet_id = (!empty($this->request->data()['pid'])) ? $this->request->data()['pid'] : 0;
			//debug($cucet_id); return null;
            if($cucet_id === 0) {
                $this->Flash->error(__('The cucet ID selected is not a valid CUCET ID.'));
                return $this->redirect(['action' => 'candidatedetails']);
            }
			$query_string = 'select u1.username as applicant_id, c1.name as c_name, c1.f_name as f_name,
							u1.email as email, u1.mobile as mobile,
							f1.payment_id as bank_payment_id, f1.payment_date_created as fee_submit_date,
							f1.payment_amount as amount, f1.id as fee_id
							from candidates c1
							left join users u1
							on c1.user_id = u1.id 
							left join fees f1 on c1.id = f1.candidate_id
							where u1.username = \'' . $cucet_id . '\'
							group by c1.id';
			$stmt = $conn->execute($query_string);
			...
			$query_string = 'select u1.username as applicant_id, c1.name as c_name, c1.f_name as f_name,
							u1.email as email, u1.mobile as mobile, p2.name, p1.marks_A, p1.marks_B, p1.marks_total, p1.checked
							from candidates c1
							left join users u1
							on c1.user_id = u1.id 
							left join preferences_new p1 on c1.id = p1.candidate_id
                            left join programmes p2 on p1.programme_id = p2.id
							where u1.username = \'' . $cucet_id . '\'';
			$stmt = $conn->execute($query_string);
			$prefData = $stmt->fetchAll('assoc');
			$this->set('prefData', $prefData);
			
			$query_string = 'select u1.username as applicant_id, c1.name as c_name, c1.f_name as f_name,
							u1.email as email, u1.mobile as mobile, p2.name as prog_name, c2.type as category, r1.rank as merit, r1.offered_seat
							from candidates c1
							left join users u1
							on c1.user_id = u1.id 
							left join rankings r1 on c1.id = r1.candidate_id
                            left join programmes p2 on p2.id = r1.programme_id
                            left join categories c2 on c2.id = r1.final_category_id
							where u1.username = \'' . $cucet_id . '\'
                            order by r1.id asc';
			$stmt = $conn->execute($query_string);
			$rankData = $stmt->fetchAll('assoc');
			$this->set('rankData', $rankData);
			
			$query_string = 'select u1.username as applicant_id, c1.name as c_name, c1.f_name as f_name,
							u1.email as email, u1.mobile as mobile, p2.name as prog_name, l1.category_pref, l1.eligible_for_open, c2.type as locked_category
							from candidates c1
							left join users u1
							on c1.user_id = u1.id 
							left join lockseats l1 on l1.candidate_id = c1.id
							left join programmes p2 on p2.id = l1.programme_id
							left join categories c2 on c2.id = l1.final_category_id
							where u1.username = \'' . $cucet_id . '\'';
			$stmt = $conn->execute($query_string);
			$lockData = $stmt->fetchAll('assoc');
			$this->set('lockData', $lockData);
			
			$query_string = 'select u1.username as applicant_id, c1.name as c_name, c1.f_name as f_name,
							u1.email as email, u1.mobile as mobile, p2.name as prog_name, c3.type as candidate_category, 
							c2.type as seat_category, f1.payment_id, f1.payment_amount, 
							f1.payment_date_created, f1.payment_transaction_id, s1.id as seat_id
							from candidates c1
							left join users u1
							on c1.user_id = u1.id
							left join seats s1 on s1.candidate_id = c1.id
							left join fees f1 on f1.candidate_id = c1.id
							left join programmes p2 on p2.id = s1.programme_id
							left join categories c2 on c2.id = s1.category_id
							left join categories c3 on c3.id = c1.category_id
							where u1.username = \'' . $cucet_id . '\'';
			$stmt = $conn->execute($query_string);
			$seatData = $stmt->fetchAll('assoc');
			$this->set('seatData', $seatData);
        }
		$this->set('programme', $programme);
    ...
    /*public function populateseats() {
		...
		$conn = ConnectionManager::get('default');
		$query_string = 'select * from no_of_seats_revised_20180704;';
		$stmt = $conn->execute($query_string);
		$seatsallocated = $stmt ->fetchAll('assoc');
		
		//debug($seatsallocated); return null;
		$count = 1;
		foreach($seatsallocated as $key => $value){
			for($i = 0; $i < $value['no_of_seats']; $i++) {
				$seat = $this->Seats->newEntity(['programme_id' => $value['programme_id'], 'category_id' => $value['category_id'], 'seat_no' => $count++]);
	...
	//return parent::isAuthorized($user);
		...
		|| $this->request->getParam('action') === 'locksupranumeraryseat'
		|| $this->request->getParam('action') === 'lockspecialcategoryseat')) {
            return true;
        }
        ...
        if (isset($user['role']) && $user['role'] === 'exam' && ($this->request->getParam('action') === 'add' 
			...
			|| $this->request->getParam('action') === 'candidatedetails')) {
            return true;
        }
		
		if (isset($user['role']) && $user['role'] === 'examread' && ($this->request->getParam('action') === 'candidatedetails'
																	|| $this->request->getParam('action') === 'summary')) {
            return true;
        }
