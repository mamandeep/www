...
debug($ugcnet_arr);
debug($peer_arr);
debug($articleconf_arr);
debug($articleperiodical_arr);
debug($semorg_arr);
debug($image);
*/
...
<div id="contentContainer" style="width: 710px; max-width: 710px; margin-left: 0px;">
    <span style="float: right; width: 132px;"><span><img src="<?php if(!empty($image['Document']['filename'])) { echo $this->webroot . '/app/webroot/' . $image['Document']['filename']; } else { echo ""; } ?>" alt="Passport Size Photograph" height="132px" width="132px"></span><br/>
                                            <span><img src="<?php if(!empty($image['Document']['filename4'])) { echo $this->webroot . '/app/webroot/' . $image['Document']['filename4']; } else { echo ""; } ?>" alt="Signature" height="50px" width="132px"></span>
    </span>
    <div style="width: 578px">
        <div style="float: left; width: 80px;"><img src="<?php echo $this->webroot . '/app/webroot/files/cup_logo.png'; ?>" alt="Logo of Central University of Punjab" height="120px" width="80px" /></div>
...
<?php 
    ...
    ob_end_clean();
    //debug($page);//return false;
    $pdf->writeHTML($page, true, false, true, false, '');
    //return false;
    $pdf->lastPage();
    $filename = APP . 'webroot/documents' . DS . $applicant['Applicant']['id'] . '_pdf.pdf';
    if (file_exists($filename)) unlink($filename);
    $pdf->Output($filename, 'F');
    echo "The Application Form has been generated. Please <a href=\"". Router::url( '/form/post_pdf_print', true ) . "\"> click here </a>";
?>
